...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("oldIY_FE.iy") end) then
			if readfile("oldIY_FE.iy") ~= nil then
				local json = game:GetService("HttpService"):JSONDecode(readfile("oldIY_FE.iy"))
				...
				writefileCooldown("oldIY_FE.iy", defaults)
				wait()
				saves()
			end
		else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("oldIY_FE.iy", game:GetService("HttpService"):JSONEncode(update))
	end
end
...
Exit_4.MouseButton1Click:Connect(function()
	SpawnCEditor:TweenPosition(UDim2.new(0.5, -180, 0, -), "InOut", "Quart", 0.5, true, nil)
	Cmdbar_3.Text = 'Command'
	DelayNum.Text = '0'
end)
...
UserInputService.InputBegan:Connect(function(input)
	if not Cmdbar or Cmdbar:IsFocused() then return end
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'oldiy_fe.iy' or name == 'oldiy_fe' then
